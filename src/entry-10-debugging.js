...
function handleCamera() {
  ...
  if (renderer.xr.isPresenting) {
    if (xrSession === null) {
      ...
      // console.log("leftCam viewport: ", leftCam.viewport.z, leftCam.viewport.w);
  ...
  if (!ONLY_LOAD_TILES_IN_VR || in_vr) {
    ...
    if (ENABLE_TILES) {
      ...
      if (xrCamera.cameras.length > 0) {
        ...
        let tophCamWorldPos = new THREE.Vector3();
        cameraTopher.getWorldPosition(tophCamWorldPos);

        if (false && cameraTopher) {
          console.log(
            "xr: ",
            ...
            " toph: ",
            cameraTopher.position.x.toFixed(3),
            ", ",
            tophCamWorldPos.x.toFixed(3)
          );
        }
